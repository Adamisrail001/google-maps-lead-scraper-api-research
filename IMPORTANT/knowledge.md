...
| Outscraper | **DISQUALIFIED — no score** | Affordable basic extraction: $3.69/1K unique measured-per-unique on base scrape alone — most expensive basic extraction in test vs $0.54–1.67 measured for scored providers. Results (97.5% core-4 fills, 96.8% delivery on its budget-reduced 80/query batch) shown as findings only | `data/outscraper/`, scoring.md DQ table |
| ScrapingDog | **DISQUALIFIED — no score** 🔴 supersedes the same-day PENDING status: paid Lite key arrived and the full-scale run was executed 2026-09-24 | Required volume: the benchmark workload (2×10×200, ll+page mode at 14z — same coordinate-anchored input shape as Lobstr's tasks) measured a hard **~49-unique/query depth ceiling** (39–57 range) — below the 60/query cap that disqualified Google — with **fully-billed 100%-duplicate pages** served past it, no exhaustion signal (44 billed dup-pages = 22% of spend). 862 uniques vs 1,739–2,567 for scored providers. Shown as findings: **$0.23/1K unique measured** (cheapest in test), core-4 fill 97.4% (best), ~1.0s median latency. Strong shallow-lookup tool, not full extraction | `data/scrapingdog/raw/run*-llpage/`, `data/scrapingdog/reports/scale-findings.md` |
...
| **Scrapingdog** | ✅ API quality / ❌ lead fields (see elimination above) | 🔴 superseded twice — final status in the decisions table above: **→ FULL-SCALE RUN EXECUTED 2026-09-24 (paid Lite key): 862 uniques, $0.23/1K measured, 97.4% core-4 fill, but a hard ~49-unique/query depth ceiling + fully-billed duplicate pages → DISQUALIFIED on required volume**, not on lead-data absence. Record: `data/scrapingdog/reports/scale-findings.md` |
...
| Lobstr | API usage field (`credit_used: 3626`, run `e57af71d…`) × account plan rate (Growth, $50/mo ÷ 30,000 credits, user-confirmed 2026-09-23) | $6.04 Run 1 → 999 unique, incl. 974 extracted emails + details + images | **$6.05** (≈$2.66 without email extraction — 🔴 superseded 2026-09-24 by the MEASURED enrichment-off run: $1.67/1K unique, `data/lobstr/raw/speed-basic/`) |
...
**🔴 REWRITTEN 2026-09-24 (supersedes the earlier version of this section, which ranked under the v1 rubric and displayed "reference" scores for disqualified providers — retired; git history preserves it).** Scoring of record: `scoring.md`, computed by `scripts/compute_final_scorecard.py` under criteria.md's 7-criterion rubric (§4, unchanged) after the core-requirement check (Step 0). The user's job: **extract all available listings from a search — name, address, phone, opening hours — reliably and affordably.** Enrichment is bonus value inside Data Quality, never a requirement. **Disqualified providers receive no score.** 🔴 UPDATED 2026-09-24: **accuracy and freshness are now MEASURED** — ground truth built per testing-plan §10 (24 businesses returned by all four providers, truth values captured live via Places API Details, `ground-truth/sample-24.json`; comparisons in `data/<provider>/analysis/ground-truth-match.json`; manual browser spot-check pending on 3 flagged businesses). Accuracy: Lobstr 100% (180/180 comparable fields) · HasData 98.9% · Apify 98.7% (no review-count field, flagged) · Bright Data 98.3%. Freshness: 100% for all four (rating/review count matched same-day live values — all scrape live, no cache detected). Only median/p95 latency (0.8) remains zeroed for all (batch/async, N/A) — effective ceiling 9.2/10. 🔴 UPDATED again 2026-09-24 (fairness twins): Speed and Cost now use MEASURED basic-workload figures — Lobstr enrichment-off run: **45.3s wall-clock** (vs 1,711s enriched — ~97% of as-tested wall was enrichment work) and **$1.67/1K unique** (926 credits/926 uniques, 1 credit/row exact; `data/lobstr/raw/speed-basic/`); HasData basic-mode twin: **146.6s** (vs 431.3s email mode) and **$0.98/1K unique** (5,121 credits/1,276 uniques, 3 credits/row exact; `data/hasdata/raw/speed-basic/`). Bright Data's 365s was already a bare-listings timing; Apify has no separable basic mode (flagged, enriched timing used).
...
| 4 | Apify | 0.98 | 1.66 | 1.34 | 0.21 | 0.44 | 0.72 | 0.48 | **5.83** |

**Sensitivity check (house-product disclosure):** both cost framings are now fully measured — basic ($1.67 vs $0.98) and as-tested enriched ($5.82 vs $1.75); recomputing with the enriched framing gives Lobstr 7.85 vs HasData 6.88 — #1 stable under both. Speed uses like-for-like basic timings for the top three; Apify flagged (no separable basic mode) (`scoring.md`).

**Basic-extraction costs used for the Cost criterion (prompt rule: basic workload first, enrichment separate) — 🔴 updated 2026-09-24, all measured where possible:** Apify **$0.54/1K** billed — flagged: no basic-only price exists, it bills per verified-email lead · HasData **$0.98/1K unique MEASURED** (basic twin: 5,121 credits, exactly 3/row, / 1,276 uniques — supersedes the $0.74/1K-row rate-card; $1.75/1K measured in email mode) · Bright Data **~$1.50/1K** rate-card estimate, unverified (no billing visibility) · Lobstr **$1.67/1K unique MEASURED** (enrichment-off run: 926 credits, exactly 1/unique — supersedes the ≈$2.66 derived figure; $5.82/1K as-tested incl. enrichment).

### 1. Lobstr.io (house — disclosed) — 8.04 — wins Reliability (85.1% delivery, zero silent truncation), Data Quality with a perfect 2.0/2.0 (best core-4 fill 97.1%, the enrichment reference point — 45.3% email + 56.8% socials — plus 100% ground-truth accuracy 180/180 and 100% freshness), Scalability (user-set concurrency, no hidden caps found), AND Speed on the basic workload (45.3s measured, fastest in test — ~97% of its as-tested 28m31s was enrichment work). Remaining cost gap: $1.67/1K measured basic, ~3× Apify's $0.54. Margin over HasData now 0.97, sensitivity-checked above.

### 2. HasData — 7.07 — the value pick: $0.98/1K unique measured basic (2nd cheapest), 146.6s measured basic wall-clock (2nd fastest), 94.2% core fills, 41.7% email fill as bonus, 98.9% ground-truth accuracy, 100% freshness. Held back by 73.7% delivery, run-to-run field wobble (run1 rating/reviews 73% vs run2 100%), and a 5-concurrency entry-plan cap.

### 3. Bright Data — 6.06 — near-perfect fills (96.4% core, 98.3% ground-truth accuracy, 100% freshness, 87.1% delivery, 0.2% error rows) but no longer the fastest bare-listings tool (365s vs Lobstr 45.3s / HasData 146.6s on the same basic work, measured 2026-09-24) dragged down by opacity and friction: no billing visibility (cost unverifiable), docs unreachable from the test env, onboarding/geo friction, keyword-only inputs.

### 4. Apify — 5.83 — cheapest billed figure and 98.7% ground-truth accuracy (rating-only freshness basis, no review-count field) but the benchmark's worst trust findings, which hit this user's "reliably" requirement hardest: two undocumented profit-guards silently truncated 4/20 queries while reporting SUCCEEDED (🔴 worst trust finding), business_status broken on 100% of records, and structural under-delivery on low-email verticals — a listings user pays in missing listings for an economics model built around emails they don't need.
...
- **Outscraper:** fails affordable basic extraction — $3.69/1K unique measured-per-unique on base scrape alone, most expensive in test vs $0.54–1.67 measured for scored providers (enrichment would add up to +$6/1K by its published stage pricing; email stages remain untested — full-scope re-run declined 2026-09-23 on that projection). Otherwise strong on this user's job: best core-4 fills (97.5%), 96.8% delivery on its budget-reduced 80/query batch (flagged), richest default listing schema.

### DISQUALIFIED — ScrapingDog (🔴 updated 2026-09-24: full-scale run executed on the paid Lite key, supersedes the PENDING status above/below)

The full benchmark workload ran 2026-09-24 (Lite plan verified live; 2 runs × 10 sub-areas × 200 requested, ll+page mode at 14z, all 10 pages fetched — 1,000 credits measured via `/account`). **Fails required volume:** a hard ~49-unique/query depth ceiling (39–57 range) — below Google's disqualifying 60/query cap — delivering 862 uniques total where the four scored scrapers found 1,739–2,567 on identical queries. The smoke's over-billing caveat confirmed at scale: past the ceiling it serves HTTP-200, fully-billed, 100%-duplicate pages with no exhaustion signal (44 billed in this benchmark = 22% of spend). Also measured: the `start` offset barely paginates at all (~22 uniques/query, first attempt preserved in `raw/run1|run2/`). Its findings stay impressive — **$0.23/1K unique** (cheapest in test), **97.4% core-4 fill** (best measured), ~1.0s median latency, ~2 min/run — making it the value pick for *shallow* lookups (top ~40–50 per area), never full extraction. Evidence: `data/scrapingdog/raw/run*-llpage/`, `data/scrapingdog/reports/scale-findings.md`.
