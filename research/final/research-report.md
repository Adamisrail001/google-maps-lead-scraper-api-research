...
**How to read this without being misled:** the rubric measures *API quality* — and Google is, by the numbers, the best-engineered API in this test. But a person searching "Google Maps lead scraper API" is not shopping for API engineering; they are shopping for **leads**. Google is **disqualified from the rankings** on three verified grounds — its own customer agreement (Maps Platform Terms §3.2.3) prohibits storing/exporting business names and addresses, its 60-results/query ceiling prevents complete result sets (measured 10/10 at the cap), and no contact fields exist at any price — its 7.66 is reported for reference. Ranked for the searcher's actual intent: **Lobstr 6.46 · Outscraper 6.22 · Apify 4.97** (both runs pooled), with the per-intent picks in §4. (Scoring caveats — 0.8 pts unscoreable for all, wall-clock architecture artifact, Outscraper's reduced scope, house-product disclosure — travel with the table; see scorecard.md.)
