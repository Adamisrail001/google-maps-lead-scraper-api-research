"""HasData Google Maps scraper - BASIC-mode speed twin (fairness run).

Identical to the Run-1 benchmark flow and timing method (same submit->poll->
fetch pipeline, same wall-clock definition) with extractEmails OFF - the
basic-workload counterpart to Lobstr's enrichment-off 45.3s run, so the
Speed criterion compares the same bare-listings work for both.
Usage: python scripts/hasdata_basic_speed_run.py   (Run-1 queries only)
...
def main():
    run_label = "speed-basic"
    run = RUNS[0]  # Run-1 marketing agencies, same 10 sub-areas
    out = ROOT / "data" / "hasdata" / "raw" / run_label
    out.mkdir(parents=True, exist_ok=True)
    log = {"run": run_label, "industry": run["industry"], "limit": LIMIT,
           "extractEmails": False, "started": datetime.now(timezone.utc).isoformat(),
           "jobs": {}}
    ...
    for t in run["targets"]:
        ...
        body = {"keywords": [run["industry"]],
                ...
                "limit": LIMIT, "extractEmails": False}
